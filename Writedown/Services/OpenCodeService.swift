...
/// Service for managing OpenCode CLI execution lifecycle
@MainActor
class OpenCodeService: ObservableObject {
    ...
    private init() {
        detectedCLIPath = resolveOpenCodePath()
    }
    ...
    /// Execute OpenCode CLI with stream-json output format
    /// - Parameters:
    ...
    func execute(
        ...
        // Resolve CLI path
        guard let cliPath = resolveOpenCodePath() else {
            handleCLINotFound()
            throw OpenCodeError.cliNotFound
        }
        ...
        guard FileManager.default.fileExists(atPath: workingDirectory.path) else {
            ...
            throw OpenCodeError.invalidWorkingDirectory(workingDirectory.path)
        }

        // Set state to running
        state = .running

        addSystemMessage("Starting OpenCode (\(cliPath)) in \(workingDirectory.path)...")
        ...
        guard let content = noteContent, !content.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            ...
            throw OpenCodeError.launchFailed("No note content provided")
        }

        // Use run command with json format
        process.arguments = [
            "run", content,
            "--format", "json",
    ...
    private func processStreamJsonLine(_ line: String) {
        ...
        guard let messageType = json["type"] as? String else {
            ...
            if let content = json["content"] as? String {
                addOutputLine(content: content, type: .stdout)
            } else {
                addOutputLine(content: line, type: .stdout)
            }
            return
        ...
        for block in content {
            ...
            case "thinking":
                 if let text = block["thinking"] as? String {
                     appendOrAddText(text, type: .thinking)
                }
            case "tool_use":
                // handled by handleToolUseMessage or stream_event
                break
            default:
                break
            }
    ...
    /// Resolve OpenCode CLI path
    private func resolveOpenCodePath() -> String? {
        // 1. Check UserDefaults for custom path
        if let customPath = UserDefaults.standard.string(forKey: "openCodeCLIPath"),
        ...
        let commonPaths = [
            ...
            NSString(string: "~/.npm-global/bin/opencode").expandingTildeInPath,
            "/usr/bin/opencode"
        ]
    ...
    private func handleCLINotFound() {
        addSystemMessage("❌ OpenCode CLI not found")
        addSystemMessage("📦 Install with: npm install -g opencode")
        ...
        Task {
            await NotificationService.shared.sendError(
                ...
                message: "Install with: npm install -g opencode\n\nOr configure the path in Settings."
            )
        }
    }
    ...
    private func finishProcess(_ process: Process) async {
        ...
        if exitCode == 0 {
            state = .completed
            addSystemMessage("✓ OpenCode completed successfully")
        } else {
            state = .failed("Exit code: \(exitCode)")
            addSystemMessage("❌ OpenCode exited with code: \(exitCode)")
        }
...
enum OpenCodeError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .cliNotFound:
            return "OpenCode CLI not found. Install with: npm install -g opencode"
        case .launchFailed(let reason):
            return "Failed to launch OpenCode: \(reason)"
