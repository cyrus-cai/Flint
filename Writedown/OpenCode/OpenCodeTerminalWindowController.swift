//
//  OpenCodeTerminalWindowController.swift
//  Writedown
//
...
/// 独立的窗口控制器，用于展示 OpenCode Terminal
/// 窗口会居中显示，解决使用 sheet 时位置太靠右的问题
class OpenCodeTerminalWindowController: NSWindowController {
    static var shared: OpenCodeTerminalWindowController?
    ...
    private func configureWindow() {
        ...
        window.title = "OpenCode Terminal"
    ...
    private func setupContentView() {
        let terminalView = OpenCodeTerminalWindow(
    ...
    /// 便捷方法：显示 OpenCode Terminal 窗口
    static func show(noteContent: String?, noteTitle: String?, workingDirectory: URL) {
        // 如果已有窗口，先关闭
        shared?.close()

        let controller = OpenCodeTerminalWindowController(
            noteContent: noteContent,
            noteTitle: noteTitle,
            workingDirectory: workingDirectory
        )
