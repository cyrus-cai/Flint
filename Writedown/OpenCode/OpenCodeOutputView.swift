...
/// View for displaying OpenCode CLI output in real-time
struct OpenCodeOutputView: View {
    @ObservedObject var service = OpenCodeService.shared
                ...
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 2) {
                        ...
                        ForEach(service.outputLines) { line in
                            OpenCodeOutputLineView(line: line)
                                .id(line.id)
                        }
                        }
                    }
    ...
    private func permissionBanner(for request: OpenCodeService.PermissionRequest) -> some View {
        HStack(spacing: 12) {
            Image(systemName: "hand.raised.fill")
                .foregroundColor(.yellow)
                .font(.system(size: 20))
    ...
    private var emptyStateView: some View {
        VStack(spacing: 16) {
            ...
            Text("Output from OpenCode will appear here")
                .font(.caption)
                .foregroundColor(.secondary)
        }
...
struct OpenCodeOutputLineView: View {
    let line: OpenCodeService.OutputLine
    ...
    private func colorForType(_ type: OpenCodeService.StreamType) -> Color {
        switch type {
        case .stdout:
    ...
    private func backgroundForType(_ type: OpenCodeService.StreamType) -> Color {
        switch type {
        case .thinking:
