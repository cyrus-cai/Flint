...
struct RecentNote: Identifiable {
    ...
    static func loadMetadata(from url: URL, modificationDate: Date) -> RecentNote? {
        ...
        var headerData = fileHandle.readData(ofLength: 1024)
        // Trim trailing bytes that split a multi-byte UTF-8 character
        while !headerData.isEmpty && String(data: headerData, encoding: .utf8) == nil {
