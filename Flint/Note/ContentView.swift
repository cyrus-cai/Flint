...
struct ContentView: View {
    ...
    func loadNoteContent(_ content: String, fileURL: URL? = nil) {
        ...
        // Freeze window height so it stays stable while SwiftUI fills the new content.
        // Unfreeze on the next RunLoop tick, after SwiftUI has finished layout.
        WindowManager.shared.activeWindow?.freezeHeight()
        text = content
        DispatchQueue.main.async {
            WindowManager.shared.activeWindow?.unfreezeHeight()
        }
